...
**Status:** done
**Commits:** `3da39f7` (SKILL.md authored). Session-shouldered companion commits in the same session: `6d055e5` (spec authorship belatedly committed at orient time — see "Surprises and learnings" below), `9ace514` (intake finding `spec-write-leaves-specs-uncommitted` captured at operator's direction during orient).
...
- **Spec authorship was not committed by `/spec-write`** — `specs/20260517-finding-triage-skill/` was found untracked at orient. This is the second consecutive occurrence (Phase B exhibited the same pattern on 2026-05-16). Remediated at `6d055e5` before opening T-01; pattern captured as intake finding `specs/findings/20260517-spec-write-leaves-specs-uncommitted/` at commit `9ace514`. The intake's triage and route decisions are out of scope for this spec (the finding now lives in the Findings Pipeline and will be triaged via `/finding-triage` itself in a later session — possibly a fitting dogfood for Phase F if not sooner).
...
**Status:** done
**Commits:** `1a6ecc9` (synthetic fixture triaged — finding.md + journal.md edits). Spec closeout commit follows.
...
**Status:** done
**Commits:** `3562026` (LWC finding triaged — finding.md status banner + Triage section + journal.md Triaged entry). Spec closeout commit follows.
...
- **T-02 closeout flagged spec-write-leaves-specs-uncommitted (`9ace514`) and intake-template-folder-dependency (`684aaa6`) as parallel-work findings.** Neither moved during T-03. Both remain at `status: intake`, awaiting their own triage in a later session. Not raised against this spec.
...
**Status:** done
**Commits:** `7d3869f` (README extended). Spec closeout commit follows.
...
**Diff range:** `6d055e5..HEAD` (Phase C spec authorship through T-04 closeout — 12 files, +542/−39)
**Blockers:** 0
**Important:** 0
...
- Intake byte-preservation verified by `git show 1a6ecc9 -- ...test-only-signal.../finding.md` and `git show 3562026 -- ...lwc-error/finding.md` — both diffs show only status-banner and Triage-section changes; Intake blocks unchanged.
...
**Trigger.** RC-3b self-review (commit `5825e10`) advisory A-3 surfaced that two intake findings were captured via `/finding-intake` during Phase C execution ([spec-write-leaves-specs-uncommitted](../findings/20260517-spec-write-leaves-specs-uncommitted/) commit `9ace514`; [intake-template-folder-dependency](../findings/20260517-intake-template-folder-dependency/) commit `684aaa6`), but §12 Out of Scope never declared session-side captures as out-of-scope side artifacts. RC-3 review (commit `63ed943`) advisory A-3 reinforced the framing. Recommended remedy was "a small §12 Out of Scope clarification, deferrable until convenient." Bundled in this amendment: a surgical tense-tweak to the existing OQ-3/OQ-4 quoting-back bullet, now that this session's preceding amendments (2026-05-17-3 and -4 against the design spec) have satisfied that deferral.
...
- **Affected checkpoints:** RC-3b is closed (`pass with comments` per `5825e10`); this amendment ratifies A-3's recommendation in spec text. RC-3 is closed (`pass with comments` per `63ed943`); same. No re-review triggered.
