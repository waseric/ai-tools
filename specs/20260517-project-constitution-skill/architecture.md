...
Before the skill existed, repository orientation was reconstructed each time a contributor (human or AI) arrived at a repo. Stack, scope, and intent were inferred from scattered signals — READMEs of varying freshness, ambient knowledge in chat, commit archaeology. The skill was introduced in commit `49c15f0` (2026-05-14) as part of the broader `spec-design` / `project-constitution` / `spec-amend` addition to the methodology. The constitution it produces for the `ai-tools` repo itself was added in commit `a11119d` (2026-05-15).
