...
- `2ef3a98` (2026-05-14) — trilogy commit added `DESIGN_SPEC_PATH` and `CONSTITUTION_PATHS` INPUTS plus their Phase-1 orientation behavior, integrating `spec-write` with `spec-design` (upstream) and `project-constitution` (upstream).
- `312453c` (2026-05-14) — session-economy and multi-repo disciplines added across the skill family (`SPEC_REPO_ROOT`/`SPEC_TARGET_BRANCH` mention in OUTPUT FORMAT).
- `dc331f5` (2026-05-15) — path convention update: `docs/specs/<feature>.md` → `specs/YYYYMMDD-<feature-name>/feature.md` + `journal.md` companion, applied uniformly across the six lifecycle skills via the [spec-path-convention](../20260515-spec-path-convention/architecture.md) feature spec.
...
- **Repository layout convention** ([specs/tech-stack.md §48](../tech-stack.md#L48)). `specs/YYYYMMDD-<feature-name>/feature.md` + `journal.md` is the canonical output path, set by the [spec-path-convention](../20260515-spec-path-convention/architecture.md) propagation in commit `dc331f5`.
...
- **Lateral.** Sibling lifecycle skills referenced `spec-write` by name and path; all six were modified together in commit `dc331f5` when path conventions changed.
...
| **Predecessor cross-check** | [docs/spec-driven-development-prompts-conversation.md lines 17–225](../../docs/spec-driven-development-prompts-conversation.md) is the skill's design-rationale source. Every spec-side commitment in §5 of this document traces back either to a behavior in SKILL.md (current) or to a recommendation in the predecessor doc (rationale). Gaps between predecessor and SKILL.md are *evolution* (trilogy commit `2ef3a98`, session-economy commit `312453c`, path-convention commit `dc331f5`), not drift. CP-2 reads this distinction. |
...
- [docs/spec-driven-development-prompts-conversation.md](../../docs/spec-driven-development-prompts-conversation.md) lines 17–225 — the `spec-writing-prompt.md` artifact + companion design notes that became this skill. Authoritative for the skill's *design rationale*; not authoritative for current behavior (which has evolved through commits `2ef3a98`, `312453c`, `dc331f5`).
