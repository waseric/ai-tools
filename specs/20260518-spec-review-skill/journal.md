...
## 2026-07-06 — Amendment 2026-07-06-1 (forward design addition — dispatch option + verdict cues)

**Section amended:** [architecture.md §4 Execution model](./architecture.md#L101), new [§5.12 Dispatch option (`REVIEW_EXECUTION`)](./architecture.md#L265) + [§5.13 Operator cues at the verdict boundary](./architecture.md#L279), and two [§6 NFR rows](./architecture.md#L277) — paired with the master [.agents/skills/spec-review/SKILL.md](../../.agents/skills/spec-review/SKILL.md) (INPUTS `REVIEW_EXECUTION` entry, new `DISPATCH MODE` + `OPERATOR CUES` sections, one WHAT-NOT-TO-DO bullet, `lastUpdated` bump).
**Trigger:** [dispatch-execution](../20260705-dispatch-execution/architecture.md) §5.8 (spec-review dispatch option) + §5.9 (operator cues), P4 of that spec's §7 Implementation Sequencing. This is a **forward design addition** (new behavior sourced from the dispatch-execution design spec), not a retroactive description of already-shipped behavior — dispatch-execution is a sibling design authority for dispatch behavior, cited here the way [session-economy §5.4](../20260514-session-economy/architecture.md) is authoritative for the multi-repo Phase 8 paragraph.
**Reason:** Adds an optional `REVIEW_EXECUTION: inline | dispatch` mode so checkpoint reviews can run in a fresh, floor-correct `spec-reviewer` context coordinated by a thin session (isolation, not a thinner review — §5.8), and a fixed-format operator cue at the verdict boundary so a cold operator can act on a verdict without reconstructing the framework (§5.9).
**Impact summary:** No tasks (retroactive design spec; no §7 Task Breakdown). Pure additions — no existing commitment altered. `REVIEW_EXECUTION: inline` is the default, so prior behavior is unchanged; a dispatch-produced verdict is format-identical to an inline one. The `spec-reviewer` agent definition this mode spawns already ships ([.agents/agents/spec-reviewer.md](../../.agents/agents/spec-reviewer.md), dispatch-execution P3).
...
The primary structured record for this amendment lives in the driving spec's journal — [specs/20260705-dispatch-execution/journal.md](../20260705-dispatch-execution/journal.md), the **P4** closeout entry — because dispatch-execution is where P4 is planned and its DoD verified. This is the **cross-spec companion entry** on the amended skill's own journal (mirroring the 2026-05-18-3 / 2026-05-19-1 cross-skill convention: the record lives once, at the driving spec, and is cross-referenced from the touched spec). See that entry for the DoD verification, the master/deploy sync evidence, and the design decisions.
