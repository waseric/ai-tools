...
- **spec-execute had four cross-spec mappings:** retro §5.1 ↔ session-economy §5.2 (Phase 1 multi-repo detection) **(§5-enumerated)**; retro §5.8 ↔ session-economy §5.1 (Phase 8 token economy) **(§5-enumerated)**; retro §5.4 ↔ session-economy §1 + §3 + commit `312453c` (Phase 4 paired-commit strengthening) **(narrative-sourced)**; retro §5.6 ↔ session-economy §1 + §3 + commit `312453c` (Phase 6 paired-commit strengthening) **(narrative-sourced)**.
...
| [specs/20260515-spec-path-convention/architecture.md](../20260515-spec-path-convention/architecture.md) | Negative signal | Modifies spec-review's `SPEC_PATH` example via commit `dc331f5` but does not architecturally describe the skill. |
...
- Commit SHAs: `2ef3a98` (trilogy), `312453c` (session-economy), `d36e38c` (lastUpdated), `dc331f5` (path convention) — all verified against `git log`.
...
**Trigger:** CP-1 review verdict (commit `885a0d2`) raised [important] finding I1 — `§5.X` placeholder unresolved AND cited pattern does not exist at N=3.
...
**Trigger.** CP-1 review (verdict commit 885a0d2, SHA backfill 09f4f72)
...
- One spec-edit commit (`145302d`) touching both `architecture.md` files.
...
**Reason:** Aligns Design Notes rationale prose with Phase 8 body procedural prose; closes the pre-trilogy phrasing that survived commit `2ef3a98`'s Phase-body update without a parallel Design Notes refactor.
...
**Trigger.** N=5 CP-2 audit finding D-1 — Design Notes "Drift is a first-class finding" sub-block (line 209) reads: *"Resolution goes through the Amendment Protocol from `spec-execute`, not through silent acceptance."* The phrasing predates trilogy commit `2ef3a98` (2026-05-14), which extracted the Amendment Protocol from spec-execute into the new `spec-amend` skill. Phase 8 body line 181 was correctly updated at that commit (*"route them through the `spec-amend` skill"*), but the Design Notes prose was not refactored — a documentation drift from the spec-driven-development-prompts predecessor's pre-trilogy phrasing. The CP-1 audit (commit `885a0d2`) surfaced this as advisory A3; the CP-2 audit (commit `0597514`) elevated it to D-1 with route (b) — amend SKILL.md. First-of-kind for the quintet retro-spec series; CP-2 audit logged the new finding class.
...
**Impact summary:** No tasks; CP-2 already closed (commit `62dca57` 2026-05-18 20:48:27); no completed work invalidated.
**Approver:** Eric Wasgatt
