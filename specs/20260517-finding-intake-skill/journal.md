...
- Operator invoked via `/spec-write phase b` against the existing findings-pipeline working directory. Confirmed at Discovery time that Phase A is closed (RC-2 verdict committed at 0839fbc; remediation at ff56a8f).
...
**Status:** done
**Commits:** 7a856f6 (finding artifact, `find:` prefix per skill suggestion) + this closeout commit (spec/journal updates)
**Files touched:**
...
- Real finding artifact committed in `specs/findings/`: ✓ — commit 7a856f6.
...
- **Backfill T-02 SHA in §7.** Noted as a drift signal in orientation; fixed in-line during closeout rather than a separate housekeeping commit. Scope is a one-token addition (`commit d82f43c`); it brings T-02's status line into the same shape as T-01's status line; not a content change.
...
- T-01: SKILL.md authored (commit 1e60db9)
- T-02: synthetic validation exercise (commit d82f43c)
- T-03: real-signal dogfood exercise (commits 7a856f6 + 9bc5d0a)
...
**Diff range:** `1e60db9^..HEAD` (7 commits)
...
Tasks-in-scope status (per [§7 task statuses](feature.md#L221-L347)): all four done; commits 1e60db9 (T-01), d82f43c+df137b3 (T-02 + SHA backfill), 7a856f6+9bc5d0a (T-03 work + closeout), 3dd8ab8 (T-04 combined).
...
Initial drift signals: (1) SKILL.md Phase 3 step 3 + WHAT NOT TO DO both prescribe "leave/preserve commented skeletons" — produced artifacts strip them per T-05 precedent. (2) T-03 closeout commit (9bc5d0a) bundled a T-02 SHA backfill; acknowledged in journal as a one-token consistency fix.
...
Scope observation `[advisory]`: T-03 closeout (9bc5d0a) bundled T-02 status-line SHA backfill alongside T-03 closeout work. Acknowledged in journal as scope-honest, but cleaner pattern is a dedicated housekeeping commit. Logged as advisory A4.
...
- **A4 (commit hygiene):** T-03 closeout commit (9bc5d0a) bundled T-02 SHA backfill. Acknowledged in journal as scope-honest. Forward guidance: dedicated housekeeping commits for future drift-fix work. No re-run required for this checkpoint.
...
**Trigger:** RC-3a review (commit cce080f) advisory A1 — template-scaffolding "copy verbatim" prose drift; SKILL.md prose contradicted three living examples that all strip template scaffolding (T-05, T-02, T-03).
...
**Trigger:** Cascading from Amendment 2026-05-17-2 to schema spec (commit `a3517f6`), which committed scaffold-marker delimiters as the strip mechanism and templates as skill-bundled-with-host-override. This amendment brings `finding-intake` into conformance. Originating finding: [intake-template-folder-dependency](../findings/20260517-intake-template-folder-dependency/finding.md).
...
**Trigger.** Cascading from Amendment 2026-05-17-2 to [specs/20260517-findings-pipeline-schema/feature.md](../20260517-findings-pipeline-schema/feature.md) (commit `a3517f6`), which committed scaffold-marker delimiters as the strip mechanism and templates as skill-bundled-with-host-override at the schema level. The `finding-intake` skill is the principal consumer of those templates; bringing it into conformance is the next cascade step.
